...
## 하단 메뉴의 콘텐츠 가림 수정 (2026-09-17)

- 내 정보의 카드에 적용된 `top: 5rem`이 모바일에서도 Bootstrap의 상대 위치 카드에 작용해, 실제 레이아웃보다 80px 아래에 표시되는 문제를 재현했습니다. 계정 카드와 강의 상세·시험 안내 카드의 위치 보정은 PC의 고정 배치가 적용되는 992px 이상에서만 사용합니다.
- 공통 하단 메뉴는 `ResizeObserver`로 안전 영역을 포함한 실제 높이를 측정해 페이지 끝 여백과 스크롤 여백에 반영합니다. 글자 확대, 줄바꿈, 화면 회전, 키보드·전체 화면에 따른 메뉴 표시 변경에도 높이를 갱신합니다. JavaScript가 없을 때는 기존 안전 영역 계산을 포함한 기본 여백을 사용합니다.
- 검증: 계정 관련 Django 28개 테스트, Chromium에서 320/390/412/767px의 내 정보·탈퇴 요청·비밀번호 변경·강의 상세·내 강의실·과정·영상·홈·설치 안내 36개 화면 조합. 마지막 카드가 본문 밖으로 밀리지 않는지, 하단 메뉴 위로 끝까지 스크롤되는지, 탈퇴 요청 버튼을 누를 수 있는지 확인했습니다. 확대 글자·큰 메뉴 높이·추가 안전 영역 모사, 가로/세로 전환, 키보드 표시 상태, JavaScript 비활성 상태도 확인했습니다. 실제 모바일 기기의 키보드와 안전 영역 동작을 직접 시험한 것은 아닙니다.

## 관리자 모바일 가로·세로 배치 수정 (2026-09-17)

- Django의 `#container > .main` 규칙이 맞춤 그리드보다 우선 적용되어 관리 메뉴 버튼과 본문이 가로로 놓이는 문제를 수정했습니다. 추가 반응형 스타일은 Django 페이지별 CSS와 반응형 CSS 뒤에 로드합니다.
- 1120px 이하에서는 관리 메뉴와 필터를 접어서 시작하고 필요할 때 펼칩니다. 관리 메뉴는 키보드로도 여닫는 기본 `details` 요소를 사용합니다. JavaScript가 없으면 메뉴·필터 내용을 처음부터 표시합니다. PC로 전환하면 메뉴와 필터를 다시 펼치며, 관리자 팝업은 본문 한 열을 유지합니다.
- 모바일의 검색·검색 결과 건수·일괄 작업 선택 상자에 필요한 폭을 확보하고, 프로그램 제목은 줄바꿈합니다. 넓은 목록은 표 영역 안에서 좌우 스크롤하며 안내 문구와 키보드 접근을 제공합니다. 세로 화면의 통계 카드는 두 열로 표시합니다.
- 검증: 관리자 관련 Django 테스트 12개 통과. Chromium에서 320/412/915/1024/1121/1440px의 대시보드·수강 신청·재수강 신청·회원 목록·강의 목록·수강 신청 편집 36개 화면 조합, 메뉴·필터 열기, 검색, 표 좌우 스크롤, 가로/세로·PC 전환, 팝업 및 JavaScript 비활성 동작을 확인했습니다. 실제 휴대폰 대신 브라우저의 모바일 화면 모사를 사용했습니다.

### 항목이 있는 운영 목록의 추가 수정

모바일 운영의 접속 보안·다운로드 목록에서 세 번째 자식인 보조 정보가 자동으로 첫 번째 그리드 열에 들어가 배지 열을 늘리고 본문을 밀어내는 현상을 재현했습니다. 공통 운영 목록의 배지·본문·보조 정보 열을 명시하고 긴 문자열 줄바꿈을 적용했습니다. 같은 구조의 승인 대기·재수강·변환·메일 목록과 회원별 다운로드 목록에도 적용됩니다. 메뉴가 없는 운영 페이지는 본문이 전체 그리드 폭을 사용하도록 수정했습니다.

검증에는 격리된 로컬 DB의 보안·다운로드 기록과 긴 자료명·IPv6 주소를 사용했습니다. 모바일 운영·회원 학습 보고서·대시보드에서 320/412/915/1440px, 기본/200% 글자 크기의 24개 조합으로 배지 폭·본문 줄바꿈·보조 정보·가로 넘침을 확인하고, 기존 관리자 36개 화면 조합도 다시 확인했습니다.
